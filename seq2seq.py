...
# metadata, idx_q, idx_a = data.load_data(PATH='data/cornell_corpus/')
# Cornell Moive
from_filenames = []
to_filenames = []
trainX = []
...
trainY = []
testY = []
from_filenames = sorted(glob.glob("*.from"))
to_filenames = sorted(glob.glob("*.to"))

with codecs.open(from_filenames[1], "r", "utf-8") as book_file:
        trainX += book_file.read()
with codecs.open(from_filenames[0], "r", "utf-8") as book_file:
        testX += book_file.read()
with codecs.open(to_filenames[1], "r", "utf-8") as book_file:
        trainY += book_file.read()
with codecs.open(to_filenames[0], "r", "utf-8") as book_file:
        testY += book_file.read()
